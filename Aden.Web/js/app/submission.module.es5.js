...
function createSubmissionGridActionButtons(container, options) {
    ...
    if (submissionStateId === 1 && hasAdmin) {
        if (canStartReport) {
            ...
            lnk += '<button class="btn btn-default btn-sm btn-grid" data-start data-submission-id=' + submissionId + ' disabled="true" data-toggle="tooltip" data-title="Missing action on specification" title="Missing action on specification" data-placement="left"><i class="fa fa-spinner fa-spin hidden"></i> Start</button>&nbsp;';
        }
